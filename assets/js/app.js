...
function initNaverMap() {
  if (mapInitialized) {
    if (kakaoMap && kakaoMap.relayout) setTimeout(function(){ kakaoMap.relayout(); }, 80);
    return;
  }
  const mapEl = document.getElementById('naverMap');
  if (!mapEl) {
    console.warn('[KakaoMap] #naverMap 요소가 아직 로드되지 않았습니다. 페이지 조각 로드 후 다시 시도합니다.');
    setTimeout(initNaverMap, 120);
    return;
  }
  ...
  if (typeof kakao.maps.Map !== 'function') {
    console.log('[KakaoMap] SDK autoload=false 상태 — kakao.maps.load 후 초기화합니다.');
    try { kakao.maps.load(function(){ setTimeout(initNaverMap, 50); }); } catch(e) { console.error('[KakaoMap] load 실패:', e); }
    return;
  }
...
function securityShowTab(tab) {
  ...
  ['admin','driver','status'].forEach(function(t) {
...
function openQrPrintWindow(items, blankCount) {
  ...
  function makeBizUrl(bizId, bizName) {
    var base = 'https://cocodss7-cell.github.io/HIVEOIL/';
  ...
  var html = '<!DOCTYPE html>'
    ...
    /* A4 페이지: 210mm × 297mm — QRP-3990: 4열 × 7행 = 28칸 */
    + '.qr-page { width: 210mm; height: 297mm; background: #fff; margin: 0 auto 24px; padding: 8.5mm 25mm; display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(7, 1fr); gap: 0; page-break-after: always; box-shadow: 0 4px 24px rgba(0,0,0,0.1); position: relative; }'
    + '.qr-page:last-child { page-break-after: auto; }'
    /* 각 라벨 칸: 40×40mm */
    + '.qr-cell { width: 40mm; height: 40mm; display: flex; flex-direction: column; align-items: center; justify-content: center; overflow: hidden; padding: 1mm; position: relative; }'
    + '.qr-cell-empty { background: transparent; }'
    + '.qr-cell img { width: 32mm; height: 32mm; display: block; image-rendering: -webkit-optimize-contrast; image-rendering: crisp-edges; }'
    + '.qr-label { font-size: 7pt; font-weight: 700; color: #0F2419; text-align: center; line-height: 1.15; margin-top: 1mm; max-width: 38mm; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; letter-spacing: -0.02em; }'
    /* 인쇄용 미디어 쿼리 */
    + '@media print {'
    + '  body { background: #fff; }'
    ...
    + 'window.addEventListener("load", function() {'
    + '  console.log("[QR Print] 페이지 로딩 완료. 인쇄 버튼을 눌러주세요.");'
    + '});'
    + '<\/script>'
    + '</body></html>';
